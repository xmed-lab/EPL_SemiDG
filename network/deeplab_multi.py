...
class ResNetMulti(nn.Module):
    def __init__(self, block, layers, input_classes, num_classes, use_se = False, train_bn = False, norm_style = 'bn', droprate = 0.1):
        self.inplanes = 64
        self.train_bn = train_bn
        super(ResNetMulti, self).__init__()
        self.conv1 = nn.Conv2d(input_classes, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
    ...
    def forward(self, x):
        ...
        upsample = nn.Upsample(size= crop_size, mode='bilinear', align_corners=True)
        x1 = upsample(x1)
        x2 = upsample(x2)
        return x1, x2
...
def DeeplabMulti(input_classes=1, num_classes=21, use_se = False, train_bn = False, norm_style = 'bn', droprate = 0.1):
    model = ResNetMulti(Bottleneck, [3, 4, 23, 3], input_classes, num_classes, use_se = use_se, train_bn = train_bn, norm_style = norm_style, droprate = droprate)
    return model
...
if __name__ == "__main__":
    ...
    deeplab = DeeplabMulti(input_classes=1,num_classes=4)
    pred1, pred2 = deeplab(x)
    with open('./model_muti.txt', 'wt') as f:
        print(deeplab, file=f)
    # pred1 = upsample(pred1)
    # pred2 = upsample(pred2)
    print(x.shape)
    print(pred1.shape)
    print(pred2.shape)

    # cla = Classifier_Module(1024, [6, 12, 18, 24], [6, 12, 18, 24], 4)
    # print(cla)
